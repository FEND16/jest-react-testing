...
it('button style should be disabled on click, check style', () => {
  const wrapper = mount(<Button onClick={() => {}} />);
  expect(wrapper.find('[data-test="button"]').hasClass('cursor-not-allowed')).toEqual(false);
  wrapper.find('[data-test="button"]').simulate('click');
  expect(wrapper.find('[data-test="button"]').hasClass('cursor-not-allowed')).toEqual(true);
})
